...
// @desc    Get dashboard statistics (Demo Mode)
// @access  Private
router.get('/stats', auth, async (req, res) => {
    try {
        ...
        if (req.user.role === 'student') {
            stats.currentGPA = 3.2;
            stats.classRank = 15;
            stats.attendanceRate = 90;
            stats.pendingFees = 25000;
        }

        if (req.user.role === 'teacher') {
            stats.myClasses = 3;
            stats.studentsCount = 120;
            stats.pendingGrades = 15;
        }
...
// @desc    Get recent activities (Demo Mode)
// @access  Private
router.get('/activities', auth, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            data: filteredActivities.slice(0, 5)
        });

    } catch (error) {
        console.error('Demo dashboard activities error:', error);
        res.status(500).json({
            success: false,
...
// @desc    Get user notifications (Demo Mode)
// @access  Private
router.get('/notifications', auth, async (req, res) => {
    try {
        ...
        console.error('Demo dashboard notifications error:', error);
        res.status(500).json({
            success: false,
...
// @desc    Get quick statistics for widgets (Demo Mode)
// @access  Private
router.get('/quick-stats', auth, async (req, res) => {
    try {
        ...
        console.error('Demo quick stats error:', error);
        res.status(500).json({
            success: false,
